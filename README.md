...
- **Multiple AI providers** — choose per-install which model does the work:
  ...
  - Any self-hosted OpenAI API compatible server (llama.cpp server, vLLM,
    text-generation-webui, LocalAI, ...) — point the plugin at your own URL,
...
- **Local backend type** — choose whether the plugin auto-discovers models
  ...
  server and mlx_lm.server — check your own server's docs, since this varies
  by project).
