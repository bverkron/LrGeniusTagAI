...
-- (e.g. llama.cpp server, vLLM, text-generation-webui, LocalAI, ...) at
-- prefs.openaiApiUrl. Unlike Ollama/LM Studio, the standard /v1/models
-- response does not reliably report vision capability across server
-- implementations, so every returned model is listed here; make sure the
-- model you pick actually supports image input.
...
function OpenAiApiAPI:doRequest(filePath, task, systemInstruction, generationConfig)
    ...
    -- Global toggle (Prompts section): lets the user skip sending the
    -- plugin's built-in system prompt, in case their model/backend
    -- handles a system role poorly. Applies to every backend, not just
    -- this one.
    if prefs.includeSystemPrompt then
